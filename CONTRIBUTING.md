...
- Clone the Auth [repository](https://github.com/linkly-id/auth)

```zsh
git clone https://github.com/linkly-id/auth
```
...
```
DATABASE_URL="postgres://linkly_auth_admin:root@localhost:5432/postgres"
```
...
Also, you must generate a JWT with the signature which has the `linkly_admin` role (or one that is specified in `GOTRUE_JWT_ADMIN_ROLES`).
...
```
// file: test.env
DATABASE_URL="postgres://linkly_auth_admin:root@localhost:7432/postgres" 👈 set to your port
```

```
// file: migrate.sh
export GOTRUE_DB_DATABASE_URL="postgres://linkly_auth_admin:root@localhost:7432/$DB_ENV"
```
...
- If the PR is a `feat`, an [RFC](https://github.com/linkly-id/rfcs) or a detailed description of the design implementation is required. The former (RFC) is preferred before starting on the PR.
