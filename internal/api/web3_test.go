...
func TestWeb3(t *testing.T) {
	...
	// Add https://linkly.id (without trailing slash) to allow list
	config.URIAllowList = append(config.URIAllowList, "https://linkly.id")
	...
	g := glob.MustCompile("https://linkly.id", '.', '/')
	config.URIAllowListMap["https://linkly.id"] = g
...
func (ts *Web3TestSuite) TestValidationRules_URINotAllowed() {
	...
	examples := []struct {
		...
		{
			chain:     ChainEthereum,
			message:   "linkly.green wants you to sign in with your Ethereum account:\n0x2F20F5A9089f09da8Cf8Ee98a65B8092b4c93506\n\nStatement\n\nURI: https://linkly.green/\nVersion: 1\nIssued At: 2025-03-29T00:00:00Z\nExpiration Time: 2025-03-29T00:10:00Z",
