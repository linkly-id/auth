...
FLAGS=-ldflags "-X github.com/linkly-id/auth/internal/utilities.Version=`git describe --tags`" -buildvcs=false
ifdef RELEASE_VERSION
	FLAGS=-ldflags "-X github.com/linkly-id/auth/internal/utilities.Version=v$(RELEASE_VERSION)" -buildvcs=false
endif
...
test: build ## Run tests.
	./go.sh test $(CHECK_FILES) -coverprofile=coverage.out -coverpkg ./... -p 1 -race -v -count=1
	./hack/coverage.sh
